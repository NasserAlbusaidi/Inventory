    ...
    <div class="max-w-full mx-auto px-2 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div class="w-full sm:w-auto flex flex-col sm:flex-row items-center gap-3">
                <a href="{{ route('sales-orders.import') }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    <label for="channelFilter"
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Channel</label>
                            ...
                            <td colspan="7"
                                ...
                                <div class="flex flex-col items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        ...
                                        <path stroke-linecap="round" stroke-linejoin="round"
                                            d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c.51 0 .962-.343 1.087-.835l.383-1.437M7.5 14.25V5.106c0-.621.504-1.125 1.125-1.125h9.75c.621 0 1.125.504 1.125 1.125v9.144M7.5 14.25h11.218c.51 0 .962-.343 1.087-.835l.383-1.437a1.875 1.875 0 0 0-1.087-2.335H11.25M-1.5 6h15" />
                                    </svg>
            ...
            <div class="md:hidden">
                <div class="px-2 py-2 space-y-3">
                    @forelse ($salesOrders as $so)
                        ...
                        <div
                            class="relative bg-white dark:bg-gray-800/50 p-4 rounded-lg shadow ring-1 ring-gray-200 dark:ring-gray-700">
                            <div class="flex justify-between items-start">
                                    ...
                                    <div class="flex justify-between items-start mb-2">
                                        <div class="flex-1">
                                            ...
                                            <p class="text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{ $so->customer_details['name'] ?? 'N/A' }}</p>
                                    ...
                                    <div class="border-t border-gray-200 dark:border-gray-700 pt-2 mt-2">
                                        <div class="flex justify-between items-center text-sm">
                                            ...
                                            <p class="font-semibold font-mono text-gray-800 dark:text-gray-200">OMR
                                                {{ number_format($so->total_amount, 2) }}</p>
                                        </div>
                                        <div class="flex justify-between items-center text-sm mt-1">
                                            <p class="text-gray-500 dark:text-gray-400">Channel</p>
                                            <p class="font-medium text-gray-700 dark:text-gray-300">
                                                {{ $so->SalesChannel->name ?? 'N/A' }}</p>
                                        </div>
                                    </div>
                                </a>
                                ...
                                <div x-data="{ open: false }" class="absolute top-2 right-2">
                                    <button @click="open = !open"
                                        class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 p-1 -m-1 rounded-full">
                                    ...
                                    <div x-show="open" @click.away="open = false" x-transition
                                        class="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-900 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 z-20">
                                        <a href="{{ route('sales-orders.edit', $so) }}"
                                            class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800">View/Edit</a>
                                        @if (in_array($so->status, ['pending', 'cancelled']))
                                            <button wire:click="deleteSalesOrder({{ $so->id }})"
                                                wire:confirm="Are you sure?"
                                                class="w-full text-left block px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-800">Delete</button>
                                        @endif
                                    </div>
                                </div>
                            </div>
