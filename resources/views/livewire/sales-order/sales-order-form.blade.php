            ...
            <div class="bg-white dark:bg-gray-800 shadow-xl rounded-xl">
                <div class="p-4 sm:p-6 md:p-8">
                    ...
                    <div>
                        ...
                        @if (!$location_id)
                            <div class="mt-4 p-4 bg-yellow-50 dark:bg-yellow-800/50 border-l-4 border-yellow-400 text-yellow-700 dark:text-yellow-200 rounded-lg">
                        ...
                        <div class="mt-4 space-y-4">
                            @foreach ($items as $index => $item)
                                {{-- Line Item Card --}}
                                <div wire:key="so-item-{{ $index }}" class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg border border-gray-200 dark:border-gray-700">
                                    <div class="space-y-4">
                                        <div class="flex justify-end">
                                             <button type="button" wire:click="removeItem({{ $index }})" class="text-gray-400 hover:text-red-500 dark:hover:text-red-400 p-1 -m-1 rounded-full hover:bg-red-100 dark:hover:bg-red-900/50 transition">
                                        ...
                                        <div>
                                            ...
                                            <select wire:model.live="items.{{ $index }}.selected_item_key" id="items.{{ $index }}.selected_item_key" class="form-select mt-1 block w-full text-sm rounded-lg" {{ !$location_id ? 'disabled' : '' }}>
                                                ...
                                                @if ($salesOrderInstance?->exists && !empty($items[$index]['selected_item_key']) && !$allSellableItems->firstWhere('key', $items[$index]['selected_item_key']))
                                                    <option value="{{ $items[$index]['selected_item_key'] }}">{{ $items[$index]['display_name'] ?? 'Previously Selected Item' }} (Unavailable)</option>
                                                @endif
                                        ...
                                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            ...
                                            <div>
                                                <label for="items.{{ $index }}.price_per_unit" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Price/Unit (OMR)</label>
                                        ...
                                        <div class="border-t border-gray-200 dark:border-gray-700 pt-3 mt-2">
                                            <div class="flex justify-between items-center">
                                                ...
                                                <span class="text-sm font-mono font-semibold text-gray-800 dark:text-gray-200">
                                                    OMR {{ number_format((float)($item['quantity'] ?? 0) * (float)($item['price_per_unit'] ?? 0), 2) }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                ...
                <div class="bg-gray-50 dark:bg-gray-800/50 px-4 sm:px-6 py-4 rounded-b-xl border-t border-gray-200 dark:border-gray-700">
                    <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
                        ...
                        <button type="submit" wire:loading.attr="disabled"
                            ...
                            <svg wire:loading wire:target="saveSalesOrder" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
