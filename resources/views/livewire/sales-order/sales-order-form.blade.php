<div class="container mx-auto p-4">
    ...
    <form wire:submit.prevent="saveSalesOrder" class="bg-white p-6 md:p-8 rounded-lg shadow-md">
        ...
        <fieldset class="border border-gray-300 p-4 rounded-md mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                <div>
                    ...
                    <input type="date" id="order_date" wire:model.defer="order_date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2">
                ...
                <div>
                    ...
                    <select id="channel" wire:model.live="channel" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 h-10">
                        @foreach($soChannels as $chan)
                            <option value="{{ $chan }}">{{ ucfirst($chan) }}</option>
                        @endforeach
                    </select>
                    @error('channel') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                </div>
                @if ($channel === 'boutique')
                <div>
                    <label for="location_id" class="block text-sm font-medium text-gray-700 mb-1">Location (Boutique)</label>
                    <select id="location_id" wire:model.defer="location_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 h-10">
                ...
                @endif
                 <div>
                    ...
                    <select id="status" wire:model.defer="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 h-10">
        ...
        <fieldset class="border border-gray-300 p-4 rounded-md mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                <div>
                    ...
                    <input type="text" id="customer_name" wire:model.defer="customer_name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2">
                ...
                <div>
                    ...
                    <input type="email" id="customer_email" wire:model.defer="customer_email" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2">
                ...
                <div>
                    ...
                    <input type="tel" id="customer_phone" wire:model.defer="customer_phone" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2">
        ...
        <fieldset class="border border-gray-300 p-4 rounded-md mb-6">
            ...
            @error('items') <div class="text-red-500 text-xs mt-1 mb-3">{{ $message }}</div> @enderror

            <div class="space-y-4">
                 @foreach ($items as $index => $item)
                    <div wire:key="so-item-{{ $index }}" class="p-3 bg-gray-50 rounded-md border border-gray-200 grid grid-cols-12 gap-x-4 gap-y-2 items-start">
                        <div class="col-span-12 md:col-span-4">
                            <label for="items.{{ $index }}.product_variant_id" class="block text-xs font-medium text-gray-700">Product Variant</label>
                            <select wire:model.live="items.{{ $index }}.product_variant_id" id="items.{{ $index }}.product_variant_id"
                                ...
                                @foreach($allProductVariants as $variant)
                                    <option value="{{ $variant->id }}">{{ $variant->full_name_with_variant }} (Stock: {{ $variant->stock_quantity }})</option>
                                @endforeach
                            </select>
                            @if(!empty($item['product_variant_id']) && $item['available_stock'] < ($item['quantity'] ?? 1) && $item['available_stock'] > 0)
                                <p class="text-yellow-600 text-xs mt-1">Low stock: only {{ $item['available_stock'] }} available.</p>
                            @elseif(!empty($item['product_variant_id']) && $item['available_stock'] == 0)
                                <p class="text-red-600 text-xs mt-1">Out of stock!</p>
                            @endif
                            @error('items.'.$index.'.product_variant_id') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>

                        <div class="col-span-6 md:col-span-2">
                            <label for="items.{{ $index }}.quantity" class="block text-xs font-medium text-gray-700">Quantity</label>
                            <input type="number" wire:model.live="items.{{ $index }}.quantity" id="items.{{ $index }}.quantity" min="1" {{ empty($item['product_variant_id']) || $item['available_stock'] == 0 ? 'disabled' : ''}}
                        ...
                        <div class="col-span-6 md:col-span-3">
                            <label for="items.{{ $index }}.price_per_unit" class="block text-xs font-medium text-gray-700">Selling Price/Unit (OMR)</label>
                            <input type="number" step="0.001" wire:model.live="items.{{ $index }}.price_per_unit" id="items.{{ $index }}.price_per_unit" min="0"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2">
                         ...
                         <div class="col-span-12 md:col-span-2">
                            ...
                            <input type="text" readonly value="{{ number_format(($item['quantity'] ?? 0) * ($item['price_per_unit'] ?? 0), 2) }}"
                        ...
                        <div class="col-span-12 md:col-span-1 flex items-center justify-end pt-5">
                            @if (count($items) > 0) {{-- Show remove button even for one item, can be re-added --}}
