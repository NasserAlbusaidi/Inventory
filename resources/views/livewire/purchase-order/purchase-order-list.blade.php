<div class="py-8">
    ...
    <div class="max-w-full mx-auto px-2 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 bg-yellow-50 dark:bg-yellow-800/50 border-l-4 border-yellow-400 dark:border-yellow-600 p-4 shadow-md rounded-md" role="alert">
            <div class="flex">
                <div class="flex-shrink-0"><svg class="h-5 w-5 text-yellow-500 dark:text-yellow-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.347 0-2.189-1.458-1.515-2.625L8.485 2.495zM10 5a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 5zm0 9a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd" /></svg></div>
                <div class="ml-3"><p class="text-sm text-yellow-700 dark:text-yellow-200">{{ session('warning_negative_stock') }}</p></div>
            </div>
        </div>
        @endif
        ...
        <div class="mb-6 bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="search" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Search Purchase Orders</label>
                    <div class="relative">
                        ...
                        <input type="text" id="search" wire:model.live.debounce.300ms="search" placeholder="Search by PO # or Supplier name..."
                ...
                <div>
                    ...
                    <select id="statusFilter" wire:model.live="statusFilter" class="form-select block w-full py-2 px-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 rounded-lg shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-gray-900 dark:text-gray-200">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-xl rounded-xl overflow-hidden">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 hidden md:table">
                <thead class="bg-gray-50 dark:bg-gray-700/50">
                    <tr>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">PO #</th>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse ($purchaseOrders as $po)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{{ $po->supplier->name ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{{ $po->order_date ? \Carbon\Carbon::parse($po->order_date)->format('d M Y') : '-' }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300 text-right font-mono">{{ number_format($po->total_amount, 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                                <a href="{{ route('purchase-orders.edit', $po) }}" class="text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 font-semibold">View/Edit</a>
                                <button wire:click="deletePurchaseOrder({{ $po->id }})" wire:confirm="Are you sure?" class="text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 font-semibold">Delete</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-sm text-gray-500 dark:text-gray-400">
                                <div class="flex flex-col items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-12 h-12 text-gray-400 dark:text-gray-500 mb-3"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 7.5V6.108c0-1.135.845-2.098 1.976-2.192.373-.03.748-.057 1.123-.08M15.75 18H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08M15.75 18.75v-1.875a3.375 3.375 0 0 0-3.375-3.375h-1.5a1.125 1.125 0 0 1-1.125-1.125v-1.5A3.375 3.375 0 0 0 6.375 7.5v1.5a1.125 1.125 0 0 1-1.125 1.125h-1.5a3.375 3.375 0 0 0-3.375 3.375V18.75m13.5 0h-13.5" /></svg>
            ...
            <div class="md:hidden">
                <div class="px-2 py-2 space-y-3">
                    @forelse ($purchaseOrders as $po)
                        <a href="{{ route('purchase-orders.edit', $po) }}" class="block bg-white dark:bg-gray-800/50 p-4 rounded-lg shadow ring-1 ring-gray-200 dark:ring-gray-700 hover:ring-indigo-500 dark:hover:ring-indigo-400 transition">
                            <div class="flex justify-between items-start mb-2">
                                <div class="flex-1">
                                    <p class="font-bold text-indigo-600 dark:text-indigo-400">{{ $po->order_number }}</p>
                                    <p class="text-sm font-medium text-gray-800 dark:text-gray-200">{{ $po->supplier->name ?? 'N/A' }}</p>
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ $po->order_date ? \Carbon\Carbon::parse($po->order_date)->format('d M, Y') : '-' }}</p>
                                </div>
                                <div>
                                    <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full @switch($po->status)@case('ordered') bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-300 @break @case('received') bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300 @break @case('cancelled') bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300 @break @default bg-gray-100 text-gray-800 dark:bg-gray-600 dark:text-gray-200 @endswitch">
                                        {{ ucfirst(str_replace('_', ' ', $po->status)) }}
                            ...
                            <div class="border-t border-gray-200 dark:border-gray-700 pt-2 mt-2">
                                <div class="flex justify-between items-center text-sm">
                                    ...
                                    <p class="font-semibold font-mono text-gray-800 dark:text-gray-200">OMR {{ number_format($po->total_amount, 2) }}</p>
                                </div>
                            </div>
                        </a>
                    @empty
