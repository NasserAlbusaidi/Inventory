        ...
        <fieldset class="border border-gray-300 p-4 rounded-md mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                <div>
                    ...
                    <select id="supplier_id" wire:model="supplier_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 h-10">
                ...
                <div>
                    ...
                    <input type="text" id="order_number" wire:model.defer="order_number" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2">
                ...
                <div>
                    ...
                    <input type="date" id="expected_delivery_date" wire:model.defer="expected_delivery_date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2">
        ...
        <fieldset class="border border-gray-300 p-4 rounded-md mb-6">
            ...
            <div class="space-y-4">
                @foreach ($items as $index => $item)
                    <div wire:key="item-{{ $index }}" class="p-3 bg-gray-50 rounded-md border border-gray-200 grid grid-cols-12 gap-x-4 gap-y-2 items-end">
                        <div class="col-span-12 md:col-span-4">
                            ...
                            <select wire:model.live="items.{{ $index }}.product_variant_id" id="items.{{ $index }}.product_variant_id"
                                ...
                                @foreach($allProductVariants as $variant)
                                    <option value="{{ $variant->id }}">{{ $variant->full_name_with_variant }} (Cost: {{ number_format($variant->cost_price, 2) }} OMR)</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-span-6 md:col-span-3">
                            ...
                            <input type="number" step="0.001" wire:model.live="items.{{ $index }}.cost_price_per_unit" id="items.{{ $index }}.cost_price_per_unit" min="0"
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2">
                        ...
                        <div class="col-span-12 md:col-span-2">
                            ...
                            <input type="text" readonly value="{{ number_format(($item['quantity'] ?? 0) * ($item['cost_price_per_unit'] ?? 0), 2) }}"
