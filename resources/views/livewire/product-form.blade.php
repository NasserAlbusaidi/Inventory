<div class="container mx-auto p-4">
    ...
    <form wire:submit.prevent="save" class="bg-white p-8 rounded-lg shadow-md">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                <label for="category" class="block text-sm font-medium text-gray-700 mb-1">Category/Collection</label>
                <input type="text" id="category" wire:model.defer="category"
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2"
                       placeholder="e.g., Floral, Oriental, Unisex">
        ...
        @foreach ($variants as $index => $variant)
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end bg-gray-50 p-4 rounded-md mb-4 border border-gray-200">
                ...
                <div>
                    <label for="cost-price-{{ $index }}" class="block text-sm font-medium text-gray-700 mb-1">Cost Price</label>
                    <input type="number" step="0.01" id="cost-price-{{ $index }}" wire:model.defer="variants.{{ $index }}.cost_price"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2">
                    @error('variants.'.$index.'.cost_price') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="selling-price-{{ $index }}" class="block text-sm font-medium text-gray-700 mb-1">Selling Price</label>
                    <input type="number" step="0.01" id="selling-price-{{ $index }}" wire:model.defer="variants.{{ $index }}.selling_price"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2">
                ...
                @if (count($variants) > 1)
                    <div class="col-span-1 flex justify-end">
                        <button type="button" wire:click="removeVariant({{ $index }})"
                                class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            Remove
                        </button>
                    </div>
                @endif
